...
DataSetVOC::DataSetVOC(CStr &_wkDir)
{
	...
	testNum = testSet.size();
        cout<<"trainNum="<<trainNum<<"testNum="<<testNum<<endl;
}
...
void DataSetVOC::loadAnnotations()
{
	...
	for (int i = 0; i < trainNum; i++){
		if (!loadBBoxes(trainSet[i], gtTrainBoxes[i], gtTrainClsIdx[i]))
			return;
                //cout<<"trainSet[i]="<<trainSet[i]<<"gtTrainBoxes[i]="<<&gtTrainBoxes[i]<<"gtTrainClsIdx[i]="<<&gtTrainClsIdx[i]<<"  i="<<i<<endl;
}
	...
	for (int i = 0; i < testNum; i++)//{
		if(!loadBBoxes(testSet[i], gtTestBoxes[i], gtTestClsIdx[i]))
			return;
                //cout<<"testSet[i]="<<testSet[i]<<"gtTrainBoxes.size="<<gtTrainBoxes[i].size()<<"  i="<<i<<endl;}
